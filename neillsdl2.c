...
void Neill_SDL_DrawString(SDL_Simplewin *sw, fntrow fontdata[FNTCHARS][FNTHEIGHT],
                          char *str, int ox, int oy, int colour)
{
   int i=0;
   unsigned char chr;
   do{
      chr = str[i++];
      Neill_SDL_DrawChar(sw, fontdata, chr, ox+i*FNTWIDTH, oy, colour);
   }while(str[i]);
}

void Neill_SDL_DrawChar(SDL_Simplewin *sw,
                        fntrow fontdata[FNTCHARS][FNTHEIGHT],
                        unsigned char chr, int ox, int oy, int colour)
{
   unsigned x, y;
...
void setColour(SDL_Simplewin *sw, int colour){
  switch(colour){
    case 1:
  Neill_SDL_SetDrawColour(sw, 255,215,0);
  break;
  case 2:
  Neill_SDL_SetDrawColour(sw, 68, 85, 255);
  break;
  case 3:
  Neill_SDL_SetDrawColour(sw, 170, 80, 130);
  break;
  case 4:
  Neill_SDL_SetDrawColour(sw, 100, 255, 50);
  break;
  case 5:
  Neill_SDL_SetDrawColour(sw, 255,127,80);
  break;
  case 6:
  Neill_SDL_SetDrawColour(sw, 255, 40, 160);
  break;
  case 7:
  Neill_SDL_SetDrawColour(sw, 255, 80, 255);
  break;
  case 8:
  Neill_SDL_SetDrawColour(sw, 190, 100, 220);
  break;
  case 9:
  Neill_SDL_SetDrawColour(sw, 255, 10, 10);
  break;
  case 10:
